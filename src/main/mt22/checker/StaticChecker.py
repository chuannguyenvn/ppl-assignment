...
def compareType(operandLeft, operandRight):
    ...
    if typeLeft != ArrayType or typeRight != ArrayType:
...
class StaticChecker(Visitor):
    def __init__(self, ast):
        ...
        self.symbol_table = [Symbol("readInteger", FunctionPrototype([], [], IntegerType())),
                             Symbol("printInteger", FunctionPrototype(['anArg'], [IntegerType()], VoidType())),
                             Symbol("readFloat", FunctionPrototype([], [], FloatType())),
                             Symbol("printFloat", FunctionPrototype(['anArg'], [FloatType()], VoidType())),
                             Symbol("writeFloat", FunctionPrototype(['anArg'], [FloatType()], VoidType())),
                             Symbol("readBoolean", FunctionPrototype([], [], BooleanType())),
                             Symbol("printBoolean", FunctionPrototype(['anArg'], [BooleanType()], VoidType())),
                             Symbol("readString", FunctionPrototype([], [], StringType())),
                             Symbol("printString", FunctionPrototype(['anArg'], [StringType()], VoidType()))]
            ...
            for scope in param:
                for declaration in scope:
                    if declaration.name == namE:
                        if isinstance(declaration.type, FunctionPrototype):
    ...
    def check(self):
        return self.visit(self.ast, self.symbol_table[:])

    def visitProgram(self, ast, param):
        for declaration in ast.decls:
            if isinstance(declaration, FuncDecl):
                self.visitTask += [Symbol(declaration.name, FunctionPrototype([item.name for item in declaration.params], \
                                                                              [self.visit(item, param).type for item in declaration.params],
                                                                              self.visit(declaration.return_type, param)), declaration.inherit,
                                          declaration)]
            elif isinstance(declaration, VarDecl):
                self.visitTask += [Symbol(declaration.name, self.visit(declaration.typ, param))]
        for declaration in ast.decls:
            if isinstance(declaration, FuncDecl):
                ...
                param.append(self.visit(declaration, [param]))
            self.visitTask.pop(0)

        entry_point = False
        for declaration in ast.decls:
            if isinstance(declaration, FuncDecl) and declaration.name == "main" and declaration.params == [] and isinstance(declaration.return_type,
                                                                                                                            VoidType): entry_point = True
    ...
    def visitVarDecl(self, ast, param):
        ...
        if ast.init is None:
            ...
            tempParam = param;
            tempParam.append([Symbol(ast.name, self.visit(ast.typ, param))])
            ...
            if isinstance(variable_type, AutoType):
                ...
                if not isinstance(variable_type, ArrayType) or not isinstance(expression_type, ArrayType):
                    if isinstance(expression_type, AutoType):
                        if isinstance(ast.init, (Id, FuncCall)):
                            self.inferTypeForSymbol(ast.init.name, variable_type, isinstance(ast.init, FuncCall), tempParam)
                        elif isinstance(variable_type, StringType) or isinstance(variable_type, BooleanType):
                    ...
                    elif not compareType(variable_type, expression_type):
                        raise TypeMismatchInVarDecl(ast)
                else:
                    if len(expression_type.dimensions) <= len(variable_type.dimensions) and variable_type.dimensions[
                                                                                            :len(expression_type.dimensions)] == expression_type.dimensions:
                        ...
                        elif len(expression_type.dimensions) < len(variable_type.dimensions):
                            raise TypeMismatchInVarDecl(ast)
                        else:
                            if isinstance(variable_type.typ, FloatType):
                                if not (isinstance(expression_type.typ, IntegerType) or isinstance(expression_type.typ,
                                                                                                   FloatType)): raise TypeMismatchInVarDecl(ast)
                            elif not compareType(variable_type, expression_type):
                                raise TypeMismatchInVarDecl(ast)
                    else:
                        raise TypeMismatchInVarDecl(ast)
    ...
    def visitParamDecl(self, ast, param):
        for declarartion in param:
            if declarartion.name == ast.name: raise Invalid(Parameter(), declarartion.name) if declarartion.inherit == True else Redeclared(
                Parameter(), ast.name)
    ...
    def visitFuncDecl(self, ast, param):
                    ...
                    if not (len(ast.body.body) > 0 and isinstance(ast.body.body[0], CallStmt) and ast.body.body[0].name == 'preventDefault'):
                        ...
                        for index in range(len(declaration.params)):
                            ...
                            if parameter.inherit == True:
                                oldLen = len(setParentParam);
                                setParentParam.add(parameter.name)
        ...
        for index in range(len(ast.params)):
            oldLen = len(setParam);
            setParam.add(ast.params[index].name)
            if (oldLen == len(setParam)): raise Redeclared(Parameter(), ast.params[index].name)

        for index in range(len(ast.params)):
            parameter = ParamDecl(ast.params[index].name, self.visitTask[0].type.parameter_type[index], ast.params[index].out,
                                  ast.params[index].inherit)
                    ...
                    if isinstance(ast.body.body[0], CallStmt):
                        if ast.body.body[0].name == 'super':
                            ...
                            try:
                                ...
                                raise TypeMismatchInStatement(ast.body.body[0])
                        ...
                        raise InvalidStatementInFunction(ast.name)
        elif len(ast.body.body) > 0 and isinstance(ast.body.body[0], CallStmt) and (
                ast.body.body[0].name == 'super' or ast.body.body[0].name == 'preventDefault'):
            raise InvalidStatementInFunction(ast.name)

        return_type = self.visit(ast.return_type, param)
        reference_environment = param[:];
        reference_environment.insert(0, local)
    ...
    def jump(self, ast, param):
        ...
        for i in range(len(self.visitTask)):
            if isinstance(self.visitTask[i].type, FunctionPrototype) and self.visitTask[i].name == func.name:
                temp = self.visitTask[i];
                self.visitTask.pop(i);
                self.visitTask.insert(0, temp);
                temp = self.visitTask[0];
                indexFound = i
        for i in range(len(self.ast.decls)):
            if isinstance(self.ast.decls[i], FuncDecl) and self.ast.decls[i].name == temp.name:
                ...
                for index in range(len(temp.type.parameter_name)):
                    paramList += [
                        ParamDecl(temp.type.parameter_name[index], temp.type.parameter_type[index], None, self.ast.decls[i].params[index].inherit)]
    ...
    def visitBinExpr(self, ast, param):
        ...
        if isinstance(operandLeft, AutoType) and not isinstance(operandRight, AutoType):
            ...
            self.inferTypeForSymbol(ast.left.name, operandRight, isinstance(ast.left, FuncCall), param)
            ...
            operandRight = operandLeft;
            self.inferTypeForSymbol(ast.right.name, operandRight, isinstance(ast.right, FuncCall), param)
            if isinstance(ast.right, FuncCall): self.jump(ast.right, param)
        elif isinstance(operandLeft, AutoType) and isinstance(operandRight, AutoType) and ast.op in ['+', '-', '*', '/']:
            return AutoType()

        if ast.op in ['-', '+', '*', '/', '%']:
            if ast.op != '%':
                if not isinstance(operandLeft, (IntegerType, FloatType, AutoType)) or not isinstance(operandRight, (
                IntegerType, FloatType, AutoType)): raise TypeMismatchInExpression(ast)
                return FloatType() if (isinstance(operandLeft, FloatType) or isinstance(operandRight, FloatType)) else IntegerType()
            else:
                if not isinstance(operandLeft, (IntegerType, AutoType)) or not isinstance(operandRight,
                                                                                          (IntegerType, AutoType)): raise TypeMismatchInExpression(
                    ast)
                return IntegerType()

        elif ast.op in ['&&', '||']:
            if isinstance(operandLeft, AutoType):
                operandLeft = BooleanType();
                self.inferTypeForSymbol(ast.left.name, BooleanType(), isinstance(ast.left, FuncCall), param)
            if isinstance(operandRight, AutoType):
                operandRight = BooleanType();
                self.inferTypeForSymbol(ast.right.name, BooleanType(), isinstance(ast.right, FuncCall), param)
            if not isinstance(operandLeft, BooleanType) or not isinstance(operandRight, BooleanType): raise TypeMismatchInExpression(ast)
            return BooleanType()

        elif ast.op == '::':
            if isinstance(operandLeft, AutoType):
                operandLeft = StringType();
                self.inferTypeForSymbol(ast.left.name, StringType(), isinstance(ast.left, FuncCall), param)
            if isinstance(operandRight, AutoType):
                operandRight = StringType();
                self.inferTypeForSymbol(ast.right.name, StringType(), isinstance(ast.right, FuncCall), param)
            if not isinstance(operandLeft, StringType) or not isinstance(operandRight, StringType): raise TypeMismatchInExpression(ast)
            return StringType()

        elif ast.op in ['==', '!=', '<', '>', '<=', '>=']:
            if ast.op in ['==', '!=']:
                if not isinstance(operandLeft, (IntegerType, BooleanType, AutoType)) or not isinstance(operandRight, (
                IntegerType, BooleanType, AutoType)): raise TypeMismatchInExpression(ast)
            else:
                if not isinstance(operandLeft, (IntegerType, FloatType, AutoType)) or not isinstance(operandRight, (
                IntegerType, FloatType, AutoType)): raise TypeMismatchInExpression(ast)
            return BooleanType()
    ...
    def visitUnExpr(self, ast, param):
        ...
        if ast.op == '!':
            if isinstance(operand, AutoType):
                operand = BooleanType();
                self.inferTypeForSymbol(ast.val.name, BooleanType(), isinstance(ast.val, FuncCall), param)
    ...
    def visitArrayCell(self, ast, param):
        ...
        if available == True and not isinstance(res.type, ArrayType):
        ...
        if expression_dimension == array_dimension:
            return res.type.typ
        elif expression_dimension < array_dimension:
            return ArrayType(res.type.dimensions[(expression_dimension):], res.type.typ)

    # ------------------------------ STATEMENT ---------------------------------
    def calling(self, ast, param, option):
        if option != 3 and (ast.name == 'super' or ast.name == 'preventDefault'): raise TypeMismatchInExpression(
            ast) if option == 1 else TypeMismatchInStatement(ast)
        ...
        if len(ast.args) > len(func.type.parameter_type):
            raise TypeMismatchInExpression(ast) if option == 1 else TypeMismatchInStatement(ast) if option == 2 else TypeMismatchInExpression(
                ast.args[len(func.type.parameter_type)])
        elif len(ast.args) < len(func.type.parameter_type):
            raise TypeMismatchInExpression(ast) if option == 1 else TypeMismatchInStatement(ast) if option == 2 else TypeMismatchInExpression()
        ...
        for index in range(min(len(func.type.parameter_type), len(ast.args))):
            ...
            if isinstance(rhs, VoidType): raise TypeMismatchInExpression(ast) if option == 1 else TypeMismatchInStatement(
                ast) if option == 2 else TypeMismatchInExpression(ast.args[index])
            if isinstance(lhs, AutoType):
                lhs = rhs;
                func.type.parameter_type[index] = rhs
                ...
                if not isinstance(lhs, ArrayType) or not isinstance(rhs, ArrayType):
                    if isinstance(rhs, AutoType):
                        rhs = lhs;
                        self.inferTypeForSymbol(ast.args[index].name, lhs, False, param)
                    elif isinstance(lhs, FloatType):
                        if not (isinstance(rhs, FloatType) or isinstance(rhs, IntegerType)): raise TypeMismatchInExpression(
                            ast) if option == 1 else TypeMismatchInStatement(ast) if option == 2 else TypeMismatchInExpression(ast.args[index])
                    elif not compareType(lhs, rhs):
                        raise TypeMismatchInExpression(ast) if option == 1 else TypeMismatchInStatement(
                            ast) if option == 2 else TypeMismatchInExpression(ast.args[index])
                else:
                    if len(rhs.dimensions) <= len(lhs.dimensions) and lhs.dimensions[:len(rhs.dimensions)] == rhs.dimensions:
                        ...
                        elif len(rhs.dimensions) < len(lhs.dimensions):
                            raise TypeMismatchInExpression(ast) if option == 1 else TypeMismatchInStatement(
                                ast) if option == 2 else TypeMismatchInExpression(ast.args[index])
                        else:
                            if isinstance(lhs.typ, FloatType):
                                if not (isinstance(rhs.typ, IntegerType) or isinstance(rhs.typ, FloatType)): raise TypeMismatchInExpression(
                                    ast) if option == 1 else TypeMismatchInStatement(ast) if option == 2 else TypeMismatchInExpression(
                                    ast.args[index])
                            elif not compareType(lhs, rhs):
                                raise TypeMismatchInExpression(ast) if option == 1 else TypeMismatchInStatement(
                                    ast) if option == 2 else TypeMismatchInExpression(ast.args[index])
    ...
    def visitFuncCall(self, ast, param):
        return self.calling(ast, param, 1)

    def visitCallStmt(self, ast, param):
        self.calling(ast, param, 2)
    ...
    def visitAssignStmt(self, ast, param):
        ...
        if not isinstance(lhs, ArrayType) and not isinstance(lhs, FunctionPrototype):
            if isinstance(lhs, AutoType) and not isinstance(rhs, AutoType):
                lhs = rhs;
                self.inferTypeForSymbol(ast.lhs.name, rhs, isinstance(ast.lhs, FuncCall), param);
                infer = False
                for declaration in param[0]:
    ...
    def visitBlockStmt(self, ast, param):
        ...
        for declaration in ast.body:
            reference_environment = param[0][:];
            reference_environment[0] = local
            if isinstance(declaration, VarDecl):
                ...
                if isinstance(declaration, AssignStmt) or isinstance(declaration, CallStmt):
                    self.visit(declaration, reference_environment)
                else:
                    if isinstance(declaration, BlockStmt): reference_environment.insert(0, [])
                    self.visit(declaration,
                               [reference_environment, param[1], param[2], param[3] if not isinstance(declaration, BlockStmt) else False])
                    if isinstance(declaration, ReturnStmt): param[3] = True

    def visitClause(self, ast, param, stmt, inLoop=False):
        if isinstance(stmt, BlockStmt):
            reference_environment = param[0][:];
            reference_environment.insert(0, [])
            self.visit(stmt, [reference_environment, param[1] if inLoop == False else True, param[2],
                              param[3] if not isinstance(stmt, BlockStmt) else False])
        elif isinstance(stmt, AssignStmt) or isinstance(stmt, CallStmt):
            self.visit(stmt, param[0])
        else:
            self.visit(stmt, [param[0], param[1] if inLoop == False else True, param[2], param[3]])
        ...
        if param[3] == False:
            ...
            if ast.expr is None:
                ...
                if isinstance(param[2], FloatType):
                    if not isinstance(return_expression, FloatType) and not isinstance(return_expression, IntegerType): raise TypeMismatchInStatement(
                        ast)
                elif isinstance(param[2], AutoType):
                    param[2] = return_expression;
                    self.visitTask[0].type.return_type = return_expression
                elif isinstance(return_expression, AutoType):
                    return_expression = param[2];
                    self.inferTypeForSymbol(ast.expr.name, param[2], isinstance(ast.expr, FuncCall), param[0])
                elif not compareType(param[2], return_expression):
                    raise TypeMismatchInStatement(ast)
    ...
    def arrayLiteralHelper(self, ast, fullAST, param):
        if len(ast.explist) >= 1:
            firstElementType = self.visit(ast.explist[0], param) if not isinstance(ast.explist[0], ArrayLit) else self.arrayLiteralHelper(
                ast.explist[0], fullAST, param)
            for index in range(len(ast.explist)):
                literal = ast.explist[index]
                currentElementType = self.visit(literal, param) if not isinstance(literal, ArrayLit) else self.arrayLiteralHelper(literal, fullAST,
                                                                                                                                  param)
                ...
                return ArrayType([len(ast.explist)], firstElementType) if not isinstance(firstElementType, ArrayType) else ArrayType(
                    [len(ast.explist)] + firstElementType.dimensions, firstElementType.typ)
        else:
            return ArrayType([0], AutoType())

    def visitArrayLit(self, ast, param):
        return self.arrayLiteralHelper(ast, ast, param)
